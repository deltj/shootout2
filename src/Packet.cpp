 ...
 *      x^5 + x^4 + x^2 + x + 1
 *
 * Note: This table was copied from the wireshark source (crc32.c), which is
 *       released under the GNU GPL v2 (or any later version).
 */
...
Packet::Packet(const uint8_t *buf, const size_t bufLen) :
    timeOfReceipt(std::chrono::system_clock::now()),
    ifindex(-1),
    dataLength(0),
    data(nullptr)
...
Packet& Packet::operator=(const Packet& src)
{
    if(&src != this)
    {
        ...
        setData(src.data, src.dataLength);
    }
